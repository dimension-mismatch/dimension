...
void validate_expression(expression_t* exp, error_manager_t* manager, type_identifier_t* return_type);



void validate_vector_literal(expression_t* exp, error_manager_t* manager){
...
void validate_assignment(expression_t* exp, error_manager_t* manager){
  type_identifier_t empty = typeid_newEmpty();
...
void validate_block(expression_t* exp, error_manager_t* manager, type_identifier_t* return_type){
  for(int i = 0; i < exp->block.arg_c; i++){
    validate_expression(exp->block.arg_v[i], manager, return_type);
  }
}

void validate_function_call(expression_t* exp, error_manager_t* manager){
  type_identifier_t empty = typeid_newEmpty();
...
void validate_return(expression_t* exp, error_manager_t* manager, type_identifier_t* return_type){
