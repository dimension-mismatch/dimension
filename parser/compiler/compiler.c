    ...
    case EXP_VALUE_LITERAL:
      switch(expr->value_literal.type){
        case VAL_STRING:
          return literal_ir_value(0);
        case VAL_CHAR:
          return literal_ir_value((uint64_t)expr->value_literal.c);
        case VAL_FLOAT:
          return literal_ir_value((uint64_t)expr->value_literal.f);
        case VAL_INT:
          return literal_ir_value((uint64_t)expr->value_literal.i);
        case VAL_UNSIGNED:
          return literal_ir_value((uint64_t)expr->value_literal.u);
      }
    case EXP_READ_VAR: {
      int* val = get_value_from_int(var_table, expr->read_var_id);
      if(!val) return literal_ir_value(0);
      return register_ir_value(*val);
    }
    case EXP_FUNCTION_CALL: {
      trie_match_result_t match = fn_trie->matches[expr->function_call.fn_id];

      if(match.type != MATCH_FUNCTION) return literal_ir_value(0);

      function_definition_t fn = match.fndec;
      if(!fn.is_IR) return literal_ir_value(0); //TODO: compile functions into IR
...
program_t compile_program(block_t ast, pattern_trie_t* fn_trie, pattern_trie_t* type_trie){
  ...
  for(int i = fn_trie->scopes[fn_trie->scope_levels - 1]; i < fn_trie->match_count; i++){
    ...
    if(match.type == MATCH_VARIABLE){
      ...
      register_file_push(&out.registers); //TODO: determine size of variables
